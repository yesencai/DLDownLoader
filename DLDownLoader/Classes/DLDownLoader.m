...
+ (NSString *)downLoadedFileWithURL: (NSURL *)url {
    ...
    if([DLDownLoaderFileTool isFileExists:cacheFilePath]) {
...
- (void)downLoadWithURL: (NSURL *)url {
    ...
    // 文件的位置, 文件的大小
    if ([DLDownLoaderFileTool isFileExists:self.cacheFilePath]) {
        NSLog(@"文件已经下载完毕, 直接返回相应的数据--文件的具体路径, 文件的大小");
        
        if (self.downLoadInfo) {
            self.downLoadInfo([DLDownLoaderFileTool fileSizeWithPath:self.cacheFilePath]);
        }
        
        self.state = DLDownLoaderStateSuccess;
    ...
    if ([url isEqual:self.task.originalRequest.URL]) {
        // 任务存在 -> 状态
        // 状态 -> 正在下载 返回
        if (self.state == DLDownLoaderStateDowning)
        {
            return;
        }
        // 状态 -> 暂停 = 恢复
        if (self.state == DLDownLoaderStatePause)
        {
            [self resume];
            return;
        }
    ...
    [self cancel];
    // 2. 读取本地的缓存大小
    _tmpFileSize = [DLDownLoaderFileTool fileSizeWithPath:self.tmpFilePath];
...
- (void)resume {
    if (self.state == DLDownLoaderStatePause) {
       [self.task resume];
        self.state = DLDownLoaderStateDowning;
    }
}
...
- (void)pause {
     ...
     {
        [self.task suspend];
         self.state = DLDownLoaderStatePause;
     }
...
//    self.state = DLDownLoaderStateFailed;
}
...
- (void)cancelAndClearCache {
    ...
    // 删除缓存
    [DLDownLoaderFileTool removeFileAtPath:self.tmpFilePath];
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveResponse:(NSURLResponse *)response completionHandler:(void (^)(NSURLSessionResponseDisposition))completionHandler
{
    ...
    if (_tmpFileSize == _totalFileSize) {
        NSLog(@"文件已经下载完成, 移动数据");
        // 移动临时缓存的文件 -> 下载完成的路径
        [DLDownLoaderFileTool moveFile:self.tmpFilePath toPath:self.cacheFilePath];
        self.state = DLDownLoaderStateSuccess;
        // 取消请求
...
- (void)setState:(DLDownLoaderState)state {
    if (_state == state) {
        return;
    }
    _state = state;
